...
xtab_gld <- CrossTable(mat, chisq = T, fisher = T, expected = T) # save xtab for later
```
...
fisher.test(bw_mat, simulate.p.value = T)
...
* Black individuals are 25% as likely as White individuals to be in employment.
* Black children are over 50% less likely than White children to reach a good level of development at age 5.
...
* Black individuals are nearly two times more likely as White individuals to be moderately to severely depressed when they start Talking Therapies.
* Black children are over two times more likely as White children to be looked after the local authority.
* Black ward patients are nearly three times more likely than White ward patients to be physically restrained.
* Black individuals are eight times more likely than White individuals to be stopped and searched by Police.
* Black households are over twelve times as likely as White households to be homeless and living in temporary accommodation.
...
convert_to_rr_2 <- function(or, ref_prob){
  rr <- or / ((1 - ref_prob) + (ref_prob * or))
  return(rr)
}

ref_prob <- talking_therapies_df$white[1] / sum(talking_therapies_df$white)
ref_prob <- gld_df$white[1] / sum(gld_df$white)
ref_prob <- stop_search_df$White[1] / sum(stop_search_df$White)

convert_to_rr(1.85, ref_prob)
convert_to_rr(0.44, ref_prob)
convert_to_rr_2(0.44, ref_prob)

# test ci

convert_to_rr_2(8.01, ref_prob)

test <- matrix(c(101,380,493,807), 2, 2)

convert_to_rr_2(xtab_gld[["fisher.ts"]][["estimate"]], ref_prob)
convert_to_rr_2(xtab_gld[["fisher.ts"]][["conf.int"]][1], ref_prob)
convert_to_rr_2(xtab_gld[["fisher.ts"]][["conf.int"]][2], ref_prob)
k <- riskratio(test)


stop_search_df
test <- matrix(c(173548,46356,3852,8244),2,2)
convert_to_rr_2(xtab_stop_search[["fisher.ts"]][["estimate"]], ref_prob)
convert_to_rr_2(xtab_stop_search[["fisher.ts"]][["conf.int"]][1], ref_prob)
convert_to_rr_2(xtab_stop_search[["fisher.ts"]][["conf.int"]][2], ref_prob)
riskratio(test)
```
...
ggplot(plot_data, aes(ethnicity, per_100k)) + 
  ...
  xlab("Ethnicity") + ylab("Number of individuals detained via Section 136 per 100,000")
...
ggplot(plot_data, aes(ethnicity, percentage)) + 
  ...
  theme(panel.background = element_rect(fill = "white"),
        ...
        axis.line = element_line(colour = "black")) +
  geom_label(aes(label = paste(round(percentage,2), "%", sep = ""))) +
  xlab("Ethnicity") + ylab("Percentage of individuals moderately/severely depreseed \non entry to Talking Therapies")
...
ggplot(plot_data, aes(ethnicity, percentage)) + 
  ...
  theme(panel.background = element_rect(fill = "white"),
        ...
        axis.line = element_line(colour = "black")) +
  geom_label(aes(label = paste(round(percentage,2), "%", sep = ""))) +
  xlab("Ethnicity") + ylab("Percentage of children looked after by the local authority")
...
ggplot(plot_data, aes(ethnicity, percentage)) + 
  ...
  theme(panel.background = element_rect(fill = "white"),
        ...
        axis.line = element_line(colour = "black")) +
  geom_label(aes(label = paste(round(percentage,2), "%", sep = ""))) +
  xlab("Ethnicity") + ylab("Percentage of Lambeth ward patients \nsubject to physical restraint")
...
ggplot(plot_data, aes(ethnicity, percentage)) + 
  ...
  theme(panel.background = element_rect(fill = "white"),
        ...
        axis.line = element_line(colour = "black")) +
  geom_label(aes(label = paste(round(percentage,2), "%", sep = ""))) +
  xlab("Ethnicity") + ylab("Percentage of individuals subject to Police Stop and Search")
...
ggplot(plot_data, aes(ethnicity, percentage)) + 
  ...
  theme(panel.background = element_rect(fill = "white"),
        ...
        axis.line = element_line(colour = "black")) +
  geom_label(aes(label = paste(round(percentage,2), "%", sep = ""))) +
  xlab("Ethnicity") + ylab("Percentage of households in temporary accommodation")
