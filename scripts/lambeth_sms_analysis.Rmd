...
All data presented, apart from the Police Stop and Search data, were sourced by Lambeth Public Health in a previous analysis. The indicators, and the sources of their data (as reported by Lambeth Public Health), are presented in the Appendix.
...
ggplot(rr_df_2, aes(indicator, rr)) +
  ...
  geom_text(x = nrow(rr_df) + 1.5, y = reduction_factor*.9 - (reduction_factor*0.15), label = "More likely", colour = "black", size = 3.5, hjust = 1, family = "archivo") +
  ...
  geom_text(x = nrow(rr_df) + 1.5, y = -1*reduction_factor*0.9 + (reduction_factor*0.15), label = "Less likely", colour = "black", size = 3.5, hjust = 0, family = "archivo") +
...
* Black households were `r round(100 * rr_df$rr[which(rr_df$short_name == "temp_accomm")],0) - 100`% (i.e., `r round(rr_df$rr[which(rr_df$short_name == "temp_accomm")], 2)` times) more likely than White households to be statutorily homeless and living in temporary accommodation.
...
In 2017-2018, 68% of Black children reached a Good Level of Development, compared to 83% of White children. Black children. Black children were `r round(100 * (1 - rr_df$rr[which(rr_df$short_name == "gld")]), 2)`% less likely than White children to reach a Good Level of Development at age 5.
...
In 2020, the employment rate amongst Black 16-64 year olds was 60%, compared to 85% for White individuals. Black 16-64 year olds were `r round(100 * (1 - rr_df$rr[which(rr_df$short_name == "employment")]), 2)`% less likely than White 16-64 year olds to be in employment.
...
In 2018, 3.6% of Black households were statutorily homeless and in temporary accommodation, compared to 0.3% of White households. Black households were `r round(100 * rr_df$rr[which(rr_df$short_name == "temp_accomm")],0) - 100`% (i.e., `r round(rr_df$rr[which(rr_df$short_name == "temp_accomm")], 2)` times) more likely than White households to be statutorily homeless and living in temporary accommodation.
...
In 2018-2019, the rate of individuals detained under Section 136 of the Mental Health act was 60 per 100,000 for Black individuals, compared to 40 per 100,000 for White individuals. There was no difference between Black and White individuals in Section 136 detention rate.
...
In the period from September 2020 to August 2021, 15% of Black individuals in Lambeth were stopped and searched by Police, compared to 2% of White individuals. Black individuals were `r round(100 * rr_df$rr[which(rr_df$short_name == "stop_search")],0) - 100`% (i.e., `r round(rr_df$rr[which(rr_df$short_name == "stop_search")], 2)` times) more likely than White individuals to be stopped and searched by Police.
...
sources_df <- data.frame("Indicator" = c("Children reaching a Good Level of Development",
                                        "Children looked after by local authority",
                                    "Pupils achieving at least Level 4 in GCSE Maths & English",
                                   
                                    "16-64 year olds in employment",
                                    "Households that are statutorily homeless and living in temporary accommodation",
                                    "Individuals subject to detention per Section 136 of the Mental Health Act",
                    ...
                    "Source" = c("Lambeth Council, Education Research and Statistics Department",
                                 "Lambeth Council, Education Research and Statistics Department",
                                 "Lambeth Council, Education Research and Statistics Department",
                                 "Annual Population Survey (NOMIS, Office for National Statistics)",
                                 "Lambeth Council Housing Advice Team",
